...
krakenRest.api("OHLC", { pair: `${tradingSymbol}/USD`, interval: "15" }).then(data=>{
  ...
  let highPrices = data.result[`${tradingSymbol}/USD`].map(item=>{
    return item[2];
  })

  //get an array of all the low prices
  let lowPrices = data.result[`${tradingSymbol}/USD`].map(item=>{
    return item[3];
  })

  let avgPrices = data.result[`${tradingSymbol}/USD`].map(item=>{
    return item[5];
  })

  //make an array from 0 to length of highPrices
  let x = Array.from(Array(highPrices.length).keys());

  plotlib.stack([{x: x, y: highPrices, type: 'scatter'},{x: x, y: lowPrices, type: 'scatter'}, {x: x, y: avgPrices, type: 'scatter'}]);


  plotlib.stack([{x: x, y:   Taira.smoothen(avgPrices, Taira.ALGORITHMS.GAUSSIAN, 5, 5, false)   , type: 'scatter'}]);
  ...
  let formattedData = formatData(avgPrices.slice(0,710), 10);
